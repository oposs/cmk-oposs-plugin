...
**This skill does not ship workflows.** Building and testing a Checkmk plugin is this
skill's job; the GitHub Actions that run that build and cut releases belong to
[`oposs/repo-infra`](https://github.com/oposs/repo-infra), which installs one standard
release flow, changelog handling, branch protection and a `ci.yml` wired to whatever the
repository actually is.

```bash
# from inside the plugin repo
/repo-infra:check     # what is missing or out of date
/repo-infra:apply     # install it
```

repo-infra detects a Checkmk plugin from `.mkp-builder.ini` and installs a `ci.yml` whose
jobs lint `local/`, run pytest against the harness above, and build a throwaway MKP via
`oposs/mkp-builder@v2` so a package that cannot be assembled fails on the pull request
rather than at release time.

Keep a `CHANGES.md` and add entries under `## [Unreleased]` as you work — it is the single
source of truth for the version, and `assets/CHANGES.md.template` is a starting point for a
